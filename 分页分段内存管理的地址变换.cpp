...
bool SegmentMethod(){
	...
	for(int i=0;i<segment.Segmentcount;i++)
	{
	 ...
	 if(flag3==0)
	 {
	 	cout<<"您输入的数据有误，与第"<<tmp1<<"号分段冲突（基址"<<segment.address[tmp1]<<"段长"<<segment.length[tmp1]<<"）,请重新输入第"<<tmp2<<"号分段对应的基址和段长)"<<endl;
	 	return false;
	 }
	 //输出初始化数据
...
//		cout<<sum<<endl;
	...
	{
		cout<<"逻辑地址溢出,重新输入！"<<endl;
		return false;
	}
	//逻辑地址换算 
	int p=logicAddress;		//记录最开始的逻辑地址 
	int tmp;				//逻辑地址变负之前的值 
	for(int i=0;i<segment.Segmentcount;i++)
	{
	...
	cout<<"您选择了分段方式，输入的逻辑地址为"<<p<<"，其对应的段号为"<<b<<"，段偏移为"<<segdeviation<<",该段对应的基址为"<<segment.address[b]<<",物理地址为"<<physicAddress<<endl;
	cout<<"是否要继续？(y/n)"<<endl;
	char c;
	cin>>c;
	if(c=='y')
